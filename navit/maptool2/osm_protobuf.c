...
//    osm_add_tag(keybuff, valbuff);
    dbg(lvl_debug, "Got tag %s == %s", keybuff, valbuff);
}


static void process_dense(OSMPBF__PrimitiveBlock *primitive_block, OSMPBF__DenseNodes *dense) {
    int i,j=0,has_tags;
    int64_t id=0;
    int64_t lat=0,lon=0;
    int64_t changeset=0;
...
//        osm_add_node(id, lat/latlon_scale,lon/latlon_scale);
        dbg(lvl_debug,"Got node id %ld, lat%f lon %f", id, lat/latlon_scale,lon/latlon_scale);
        if (has_tags) {
...
static void process_way(OSMPBF__PrimitiveBlock *primitive_block, OSMPBF__Way *way) {
    int i;
    int64_t ref=0;
...
static void process_osmdata(OSMPBF__Blob *blob, unsigned char *data) {
    int i,j;
    ...
    for (i = 0 ; i < primitive_block->n_primitivegroup ; i++) {
        ...
        process_dense(primitive_block, primitive_group->dense);
        for (j = 0 ; j < primitive_group->n_ways ; j++)
            process_way(primitive_block, primitive_group->ways[j]);
        for (j = 0 ; j < primitive_group->n_relations ; j++)
            process_relation(primitive_block, primitive_group->relations[j]);
    }
...
int map_collect_data_osm_protobuf(FILE *in) {
    ...
    while ((header=read_header(in))) {
        ...
        if (!g_strcmp0(header->type,"OSMHeader")) {
            ...
            process_osmdata(blob, data);
        } else {
            dbg(lvl_error, "skipping fileblock of unknown type '%s'\n", header->type);
            g_free(buffer);
            return 0;
        }
        g_free(data);
